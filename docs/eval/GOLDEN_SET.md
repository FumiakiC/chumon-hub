...
- 初期10件は `TEST-01`〜`TEST-10`（A2×4・A3×5・A4×1）。拡充分は `TEST-11` 以降に連番で追加する。
- 拡充分は `TEST-11`〜`TEST-18`（2026-09-13 追加。A1×1・A2×1・A3×2・A4×4。ベクタ 3 件・ラスタ 5 件）。
...
- 評価対象とする図面本文の注記（`expected.notes` に書く文字列）は墨消しせず残す。それ以外の本文注記・寸法・形状はすべて墨消しする。
- 数量の記載が原本に無い図面（`expected.quantity` が `null` のケース）は、原本にそもそも数量記載が無いものに限る。墨消しで数量記載部を消して `null` のケースを作ってはならない（実運用に存在しない入力になるため）。
- 用紙サイズ・ページ構造は原本のまま維持する（`detectPageSize` と座標系を変えないため）。
- CAD 出力の PDF はソフトによってベクタ（文字はアウトライン化）とラスタ（ページ全体が 1 枚の画像）に分かれ、**本番入力にも両方が混在する**。golden set も両方を含め、**原本の形式を保つ**。墨消し作業で新たに画像化しない。やむを得ず画像化した場合は `memo` にその旨と解像度を明記する。
- 形式の確認方法。まず次のコマンドで画像 XObject の有無を見る。これは**目安**であり、判定を確定するものではない:

  ```bash
  for f in pdf/TEST-*.pdf; do printf "%s image=%s\n" "$f" "$(grep -a -c '/Subtype */Image' "$f")"; done
  ```

- このコマンドは `/Subtype /Image` の出現数を数えるだけなので、単独では分類を確定できない。ロゴ等の画像を 1 枚含むだけのベクタ PDF も `1` 以上になり、inline image（`BI … ID … EI`）を使うラスタや、複数の画像に分割されたラスタ、圧縮オブジェクトストリーム（`/ObjStm`）に格納された画像は数えられない。`0` ならベクタとほぼ断定でき、`1` 以上は「画像を含む」までしか言えない。
- ページ全体の形式は Acrobat で確定する。ページ上の文字や線を選択ツールでオブジェクトとして選択できればベクタ、ページ全体が 1 枚の画像として選択されればラスタである。判定結果は `memo` の `[src:vector]` / `[src:raster]` に記録する。
- 回転ケース（`/Rotate` 付き）を作る場合は、原本の形式のまま Acrobat のページ回転で付与する（画像変換と併用しない）。
...
- `notes` は図面の文字列を記載どおりに書く（要約・言い換えをしない）。判定の正規化は NFKC・連続空白の畳み込み・前後 trim のみで、英字の大小は区別される（`lib/eval/normalize.ts` の `FIELD_SPECS.notes`）。
- 複数行にわたる注記は、改行を半角スペース1つに置き換えて1行で書く（正規化で改行は空白1つに畳まれるため）。
- 注記が複数ある図面は、複数注記の `accepted` 規約が未定のため golden set に含めない（注記が1つの図面のみを選ぶ）。

### ケースのカテゴリ記録（`memo` の接頭辞）

- ケースの収載目的は `memo` の先頭に固定接頭辞 `[cat:<種別>]` で記録する。`goldenLabelSchema` は未知キーを拒否するため `tags` 等のキーは追加しない。
- `[cat:<種別>]` に続けて第 2 接頭辞 `[src:vector]` または `[src:raster]` を付ける。例: `"memo": "[cat:a4][src:raster] …"`。
- 種別と形式は独立に記録する（形式は方式比較の交絡要因になるため、集計時に切り分けられるようにする）。
- 種別の一覧:

    | 種別              | 意味                                       |
    | ----------------- | ------------------------------------------ |
    | `notes-body`      | 図面本文に注記がある（表題欄外）           |
    | `qty-null`        | 原本に数量記載が無い                       |
    | `a4`              | A4 用紙で数量記載あり                       |
    | `a1`              | A1 用紙（`CROP_SETTINGS.A1` の検証）        |
    | `surface-variant` | SOB 以外の表面処理（説明文の例示への過学習検証） |
    | `qty-distractor`  | 訂正記事など数量以外の箇所に個数を示す語がある |
    | `base`            | 初期10件（特定の目的なし）                 |
