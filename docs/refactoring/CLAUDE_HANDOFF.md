...
- Next.js **16.2.9**（App Router、ミドルウェアは **proxy.ts** 規約 / middleware.ts ではない）、React 19.2.5、**TypeScript 6**、**pnpm 10.33**、Tailwind v4、shadcn-ui(Radix)、zod、**@google/genai**（Gemini SDK。File API・生成とも一本化済み）。
...
- 抽出フロー: アップロード→`os.tmpdir()`→Gemini File API→暗号化トークン→`extract-order` が復号→`generateStructured`（`lib/ai/generate.ts`＝`generateContent`+`responseJsonSchema`+zod 検証）→`finally` でファイル削除。
...
- **次候補（申し送り）**: **Dependabot は現時点で open 0（major・minor とも全消化。radix-ui/react-slot 1.3.0 / react-hook-form 7.80.0 も merged 済み）**。**フォローアップ chore（別チャット）**: ①本番 `Dockerfile` の `npm install -g pnpm` を `pnpm@10.33.0` に pin（#197 Copilot 指摘・方針8。Dev Container 側は pin 済みで prod のみ不整合）。②dev(glibc/bookworm) と prod(musl/alpine) のベース OS 非対称を ADR 化（#197 Gemini 指摘。現状は各イメージ内 install＋CI build 検知で許容、具体 native 失敗が出るまで alpine 維持）。①②とも適用済み（#199 / ADR-4）。**refactor 本線は PR-10（#238, logger）まで完了**。PR-08（#236, error-handling）で型付きエラー基盤 `lib/errors.ts` 新設＝`AppError`/`ConfigError`/`APP_ERROR_CODES`/`getErrorCode` 型ガード/`errorResponse`・`validationErrorResponse`。`resolveError` を raw includes→code 分岐化し死に分岐 `'API_SECRET is missing'` 廃止＝#204 実バグ是正。`(error as any)` 全撲滅(grep 0件)・内部設定文言（`GOOGLE_API_KEY ...`）のクライアント露出除去・order `handleApiResponse` に成功応答 zod 防御検証。PR-09（#237）で `createRemoteJWKSet` をモジュールスコープの `Map`〈**キー=JWKS URL**＝issuer ごとに1つ〉でメモ化し jose の JWKS 内部キャッシュ(cooldownDuration 既定30s/cacheMaxAge 既定10分)を跨リクエスト有効化（挙動不変・型 `ReturnType<typeof createRemoteJWKSet>`・`as any` 不使用。コメント文言は #237 レビューで「issuer(JWKS URL)単位」→「JWKS URL（issuer ごとに1つ）単位」に修正）、次は **PR-11（fix-lint-baseline）**。PR-10（#238）で全 `console.*`27件を `lib/logger.ts` 経由化＝`[v0]`8件除去・`debug`/`info` 本番抑制、`extract-order` の復号トークン内容(`fileUri`/`name`)・`fileManagerName`・`crop-title-block`/`use-drawing-analysis` のファイル名をログ除去（`error`/`warn` は1:1温存＝挙動・スキーマ・認証/暗号ロジック不変）。ボット2件は REJECT-for-scope→下記 PR-10 由来の申し送りへ。PR-11 対象の lint 既存エラー（`use-order-processing.ts` set-state-in-effect）は #235 の全体 lint でも同一1件のみと再確認済み。**PR-06 由来の申し送り**: ボット #1(`remoteName` を `unlink` 前へ) は ACCEPT で解決済み。#2#3 の `(error as any).code` 撤廃・`as any` 回避は **PR-08（#236）で解決済み**（`crypto.ts`＝`ConfigError`／`check-document-type`＝型ガード。`(error as any)` grep 0件）。~~ただし check-document-type の handler throw 時の orphan remote cleanup は PR-08 では未対応（残置）＝route は `deleteRemoteAfter: false` のまま明示 cleanup なし~~ → **#297 で完了（2026-08-22 merged）**。`deleteRemoteAfter: boolean` を `remoteCleanup: 'always' | 'on-error' | 'never'` に置換し、throw 時に加えて **`isQuotation: false` 判定時の orphan も解放**（応答契約 `checkDocumentTypeResponseSchema` を判別可能ユニオン化し、参照先が存在しない `fileId` を返さない）。以降の申し送りは `docs/roadmap/PHASE4_PLUS_ROADMAP.md` §4「エポック0 の申し送り（#297 由来）」が正。**PR-10 由来の申し送り（フォローアップ fix）**: `crop-title-block/route.ts` は集中化した `validateUploadFile`（`instanceof File`→400／`file-type` マジックバイト→415／25MB→413）を未使用で、詐称可能な `file.type`/拡張子チェックのみ。かつ `getAll('file')` が `string` を返すと `file.type` undefined で TypeError(500) の潜在。PR-10 のボット2件(#238)は挙動変更を伴うため REJECT-for-scope とし、別 `fix/crop-title-block-validation` で per-file 適用 → **#295 で完了（2026-08-22 merged）**。設計判断は「検証失敗時は部分成功をやめて全体拒否」を採用（`validateUploadFile` が string ガードを内包／pdf-lib に渡すため `detected.mime === 'application/pdf'` 限定／pdf-lib の処理失敗時の `null` push と全滅時 400 は温存）。副産物として proxy ボディ上限の実バグを検出＝ PHASE4_PLUS_ROADMAP.md §3.2「上流制約」。**PR-05 / #204 由来の申し送り**: 1) `safeParseFloat` デッドコード整理は `fix/lint-baseline`（PR-11）で最小修正対応、2) `refactor/error-handling`（**PR-08 / #236, merged**）で**実施済み**：order 経路 `handleApiResponse` に成功応答の zod 防御検証（`schema.safeParse`）＋code 伝播を追加、#204 ボット由来3件〈`resolveError` の死に分岐 `'API_SECRET is missing'`（実 throw 文言 `'API_SECRET is not set in environment variables'` と不一致で機能せず＝実バグ）を code 分岐化で是正／フォールバック文言の内部例外 UI 露出（情報漏えい）を固定汎用文言化・`raw` は撤去し内部限定／`crypto.ts`・`app/api/check-document-type/route.ts` の `(error as any).code` を型付きエラー化〉すべて反映、3) Prettier 一括正規化は `chore/format-baseline`（#204, merged）で**完了**（コード56ファイル正規化・`*.md`/`next-env.d.ts` 除外・`.git-blame-ignore-revs` 整備。CI `prettier --check` は PR-12 相乗り）、4) `verificationSchema` の `setValueAs` が空文字で `null` を返し `z.literal('')` のカスタムエラーが出ない件は別 fix（provisional-order 入力検証）で対応（#204 ボット指摘）。**別軸フォローアップ `chore/`（EoL駆動・別チャット）: Gemini モデル 3.x 移行**（`gemini-2.5-*` shutdown 2026-10-16。中央化済み `lib/ai/models.ts` の3値差し替えのみ）。差し替え先・要測定事項（extractDrawing の精度変更）・Gemini3 の thinking 既定ON 等は `REFACTORING_PLAN.md` 付録チェックリストの「フォローアップ（PR-04 派生）」に集約。
