...
- [x] PR-06 gemini-file-helper（**#224, merged**。`lib/ai/pipeline/{types,upload,index}` 新設＝`validateUploadFile`(no-throw union) ＋ `withUploadedFile`(`deleteRemoteAfter` で remote 寿命制御)。両ルートをヘルパ化し extract-drawing にマジックバイト415・サイズ25MB(413) 統一。prompt/schema/model・token・`normalizeDrawingNo` 不変。ボット #1 `remoteName` 順序 ACCEPT／#2#3 `(error as any).code` 撤廃は PR-08 送り。詳細は PR-06 節「実績(#224)」）
