...
export function StepItem({
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div
        className={`relative flex h-14 w-14 items-center justify-center rounded-full border-2 shadow-lg transition-all duration-500 ${getContainerClasses()}`}
      >
        {/* Ping animation for current step */}
        {isCurrentStep && !isError && (
          <div
            className={`absolute inset-0 animate-ping rounded-full ${getPingClasses()}`}
          />
        )}
        <Icon
          className={`h-7 w-7 transition-all duration-500 ${getIconClasses()}`}
        />
      </div>
      <span
        className={`text-sm font-bold tracking-wide transition-colors duration-300 ${getLabelClasses()}`}
      >
        {label}
      </span>
    </div>
