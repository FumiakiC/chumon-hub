import { Brain, CheckCircle2, ShieldCheck, Upload, X } from 'lucide-react'
...
const STEP_ORDER: ProcessingStatus[] = [
  'uploading',
  'flash_check',
  'pro_extraction',
  'complete',
...
export function ProcessingStepper({ status, logs }: ProcessingStepperProps) {
  // Flash判定エラーかどうかを判定
  const isFlashCheckError =
    status === 'error' && logs.some((l) => l.message.includes('判定結果: ❌'))
        ...
        return (
          <div key={step.id} className="contents">
            ...
            {index < STEP_CONFIGS.length - 1 && (
              <ProgressBar
                isActive={
                  getStepState(STEP_CONFIGS[index + 1].id, effectiveStatus)
                    .isActive
                }
