...
| ステップ | 画面イメージ | 説明 |
| --- | --- | --- |
| 1. ホーム画面 | ![Step 1: ホーム画面](docs/images/step1-upload.png) | アプリケーションの入口。PDFや画像をドラッグ＆ドロップでアップロードするか、ファイル選択ダイアログから注文書をアップロードします。 |
| 2. 本注文書画面 | ![Step 2: 本注文書画面](docs/images/step2-processing.png) | アップロードされたドキュメントが表示されます。ユーザーが注文内容を確認できます。 |
| 3. プログレスバー | ![Step 3: プログレスバー](docs/images/step3-form.png) | Google Gemini 2.5 Flash による AI 解析の進捗をリアルタイムで表示します。「ドキュメント判定」「データ抽出」「テーブル構造化」などの各ステップを可視化し、ユーザーに処理状況を伝えます。処理完了後、抽出されたデータが注文管理フォームに自動入力されます。 |
