...
export const DRAWING_EXTRACTION_PROMPT =
  'あなたは日本の機械図面を読み取る熟練したAIです。提供された図面PDF（特に右下の表題欄）から、指定スキーマに従ってワンパスで正確に抽出してください。\n\n【厳格な抽出ルール】\n1. reasoning: まず図面全体を見て、特に数量判定（表題欄の周囲の余白の走査と、四角囲み数字の除外）と、材質・表面処理の有無判定をステップバイステップで検討し、最初に出力すること。\n2. 図面番号 (drawingNo): 図面上に表記されている文字列をそのまま抽出すること。\n3. 数量 (quantity): 『数量』や『QTY』という項目名はありません。項目名も単位も伴わず単独で書かれた整数が数量。多くは表題欄の外側・右端の『粗サ』欄の上方の余白にあるが、位置は図面により異なるため、そこに無ければ表題欄の周囲（上辺沿いの余白・右上）を走査すること。粗さ記号の有無に関わらず走査する。図面番号の横などにある『四角で囲まれた数字』は用紙サイズ等の記号なので、絶対に数量として抽出しないこと。見つからなければ null を出力し、項目名のある欄の数字（尺度・日付・公差値など）で代用しないこと。\n4. 材質 (material) と表面処理 (surfaceTreatment): 明確な記載のみ抽出すること。材質の例は SS400、SUS、S45C（括弧書きの補足があればそれも含めて記載どおりに）、表面処理の例は SOB、めっき、アルマイト。SOB は表面処理であり材質ではない。『250227』のような数字の羅列（日付）や『千葉奎耀』のような人名（設計者・製図者など）は絶対に推測で当てはめず、空欄の場合は必ず空文字（""）を出力すること。\n5. confidence: 読み取りの自信度を0〜100で評価し、材質や図番が不明瞭な場合は大きく減点すること。\n6. JSON形式のみを返すこと。'
